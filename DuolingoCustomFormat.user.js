...
//<textarea data-test="challenge-translate-input" autocapitalize="off" autocomplete="off" autocorrect="off" spellcheck="false"
//class="_7q434 _1qCW5 _2fPEB _3_NyK _1Juqt _3WbPm"
//data-gramm="false"
//dir="ltr" lang="en"
//placeholder="Escribe en inglés" style="background-repeat:
//repeat; background-image: none;
//background-position: 0% 0%;"></textarea>

var once = true;

// MutationSummary style element queries
var elementQueries = 
[{
			element: "textarea[data-test='challenge-listen-input'][placeholder='Escribe en inglés']"
},
{
			element: "textarea[data-test='challenge-translate-input'][placeholder='Escribe en inglés']"
}];
// parallel arays to above, changes
var elementChanges = 
[{
			attr : { 'placeholder' : "ENGLISH!!!" },
			css : { "border-color" : "red", "font-weight": "bold" }
},
{
		...
		for (q=0; q<elementQueries.length; ++q) {
			...
			summary.added.forEach(function(el) {
				...
				if ((typeof element != 'undefined') && (element !== null)) {
				    ...
				    manipulateTargetElement(targetElement, elementChanges[q]);
				}
			});
		}
	};

	var textareaObserver = new MutationSummary({
...
alert('manip');
	Object.keys(changesDictionary).forEach(function(changeType) {
		...
		Object.keys(changeValuesDictionary).forEach(function(changeKey) {
			if (changeType == 'attr') {
		    	$(targetElement).attr(changeKey, changeValuesDictionary[changeKey]);
		    }
			if (changeType == 'css') {
		    	$(targetElement).css(changeKey, changeValuesDictionary[changeKey]);
		    }
		});
	});
}
